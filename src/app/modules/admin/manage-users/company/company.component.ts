...
export class CompanyComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.get_all_compny_users('', (response)=>{
      setTimeout(()=>{
  ...
  search_users(){
    this.show_loader = true;
    this.get_all_compny_users(this.searchstring, (response)=>{
      setTimeout(()=>{
        this.show_loader = false;
      }, 200);
      if(response!= undefined && response['data']!= undefined){
  ...
  get_all_compny_users(search_string, callback) {
    setTimeout(() => {
      this.service.get_all_compny_users(search_string).subscribe(response => {
  ...
  public edit_function(user_id){
    let edit_router = '/admin/users/edit/company/' + user_id; 
  ...
  quick_register(template: TemplateRef<any>){
    this.popup_title = "Add Company User";
    this.action_button_text = "Save";
    this.success_message = "Company user added successfully.";
    this.show_loader = true;
    this.form_data.industry = '';
    this.form_data.account_type = 'Company';
  ...
  select_industry(){
    if(this.form_data.industry !+ -1){
      this.form_data.other_industry = '';
    }
  }   
  ...
  on_submit(isValid: Boolean){
    if (isValid){
      ...
      this.service.registration(this.form_data.firstname,this.form_data.lastname, this.form_data.email_address, this.form_data.re_enter_password, this.form_data.account_type, this.form_data.company_name, this.form_data.industry, this.form_data.other_industry).subscribe(response => {
